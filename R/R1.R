...
seq(1,20,by=2) #by mean skip or each change
seq(2,21,by=2)

rep(1,10)  #rep=repition
...
s
class(s)
# 2. Numeric vector  c=vector which is used to put multple value together
...
h
class(h)
h <- c(23, 21.3, c(2,5,3), "hello", 3+5i, 2L,TRUE)
h
class(h)
...
# of dimensions.
?array
j <- array(c("R", "Programming", "Session"), dim = c(2, 5, 4))  #2=no.of dimension 2-D ,5= no.of columns,4=repetion
